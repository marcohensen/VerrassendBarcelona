        ...
        @else
            <section>
                ...
                @foreach($hFiets as $fiets)
                    <form class="@if(count( $errors ) > 0) error @endif" method="post" action="{{ route('Admin.verwijder') }}">
                        @csrf
                        @if(count( $errors ) > 0)
                        ...
                        FietsNummer: <label>{{$fiets->id}}</label> &nbsp;&nbsp;
                        ...
                        <input type="submit" value="Verwijder Fiets">
                    </form>
                @endforeach
                <br>
                TourFietsen
